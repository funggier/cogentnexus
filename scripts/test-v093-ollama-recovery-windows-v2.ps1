...
if ($SyntaxOnly) {
    Write-Host 'CogentNexus v0.9.3 Ollama Recovery Reality harness v2 syntax/load: PASS'
    exit 0
}
...
$LogPath = Join-Path $Downloads "CNX_V093_OLLAMA_RECOVERY_V2_$Stamp.txt"
$JsonPath = Join-Path $Downloads "CNX_V093_OLLAMA_RECOVERY_V2_$Stamp.json"
...
$Evidence = [ordered]@{
    schemaVersion=3; suite='v0.9.3-ollama-recovery-reality-windows-v2'; startedAt=(Get-Date).ToString('o'); provider='ollama'; scenarios=@($Expanded)
...
function Assert-SafeKill {
    ...
    if($banned -contains $name){throw "Refusing unsafe kill target '$name'."}; if($targetPid -eq [int]$PID){throw 'Refusing to kill harness.'}
...
function Hard-Kill {
    param([ValidateSet('gateway','provider')][string]$Role,$Snapshot)
    $target=Assert-SafeKill -Role $Role -Snapshot $Snapshot; Active "inject-$Role-hard-crash" ([ordered]@{target=$target;method='Stop-Process exact PID only'})
    Stop-Process -Id ([int]$target.pid) -Force -ErrorAction Stop; Step "inject-$Role-hard-crash" 'PASS' ([ordered]@{target=$target;method='Stop-Process exact PID only'}); Clear-Active
}
...
function Assert-Baseline {
    ...
    $ok=([string]$status.state.mode -eq 'managed') -and ([string]$provider.selectedProvider -eq 'ollama') -and $adapterOkay -and [bool]$gateway.listening -and [bool]$ollama.listening
    Step "assert-managed-$Label" $(if($ok){'PASS'}else{'FAIL'}) ([ordered]@{mode=[string]$status.state.mode;selectedProvider=[string]$provider.selectedProvider;providerEventAdapter=$(if($adapter.Count -eq 1){$adapter[0]}else{$null});gateway=$gateway;ollama=$ollama})
    if(-not $ok){throw "Managed Ollama baseline failed at $Label."}
}
function Confirm-Disruptive {
    if($Disruptive.Count -eq 0){return}; Write-Host ''; Write-Host 'DISRUPTIVE OLLAMA RECOVERY REALITY TESTS REQUESTED.'; Write-Host "Scenarios: $($Disruptive -join ', ')"; Write-Host 'Only exact validated Gateway/Ollama listener PIDs may be force-killed; process trees are never killed.'
...
function Scenario-OperatorStop {
    ...
    $status=Invoke-CnxJson -Name 'status-after-intentional-stop' -CommandArgs @('status'); if([string]$status.state.mode -ne 'maintenance' -or [string]$status.state.desiredGateway -ne 'stopped' -or [string]$status.state.desiredProvider -ne 'stopped'){throw 'Intentional stop state mismatch.'}
...
function Best-Effort-Reconcile {
    if(-not(Test-Path $Cnx)){return}; try{Invoke-CnxText -Name 'cleanup-start' -CommandArgs @('start')|Out-Null;Assert-Baseline 'cleanup';Step 'cleanup-reconcile' 'PASS' ([ordered]@{provider='ollama'})}catch{Step 'cleanup-reconcile' 'FAIL' ([ordered]@{error=$_.Exception.Message})}
}

Set-Content -Path $LogPath -Value "CogentNexus v0.9.3 Ollama-only Recovery Reality Windows Harness v2`r`n" -Encoding UTF8; Save-Evidence
try {
    Invoke-Probe -Name 'openclaw-version' -Command { & openclaw.cmd --version }; Invoke-Probe -Name 'openclaw-config-validate' -Command { & openclaw.cmd config validate }; Invoke-Probe -Name 'ollama-version' -Command { & ollama.exe --version }
    if(-not(Test-Path $Cnx)){throw 'cnx.cmd is not installed.'}; Confirm-Disruptive
    foreach($name in $Expanded){switch($name){'baseline'{Assert-Baseline 'baseline';Step 'scenario-baseline' 'PASS' ([ordered]@{provider='ollama'})}'gateway-crash'{Scenario-Gateway}'provider-crash'{Scenario-Provider}'operator-stop'{Scenario-OperatorStop}}}
    $Evidence.result='PASS';$Evidence.completedAt=(Get-Date).ToString('o');Clear-Active;Save-Evidence;Log 'COGENTNEXUS v0.9.3 OLLAMA RECOVERY REALITY SUITE V2: PASS';Write-Host "Evidence: $LogPath";Write-Host "Evidence JSON: $JsonPath";exit 0
} catch {
